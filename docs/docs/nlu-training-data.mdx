...
Messages starting with `/` are sent straight to the `RegexInterpreter`, which expects NLU input in
a shortened `/intent{entities}` format.
...
NLU bypass can interfere with certain channels where it is common to receive slash commands, such as Telegram.
It can be disabled in a channel connector by setting the [UserMessage](/reference/rasa/core/channels/channel#usermessage-objects)
attribute `disable_nlu_bypass` to `True`. See the documentation for individual channel connectors to know more about their NLU bypass behavior.

When NLU bypass is enabled, to send a message with a forward slash to the NLU interpreter instead of regex interpreter
just add an extra forward slash at the start. For example:

`//send_to_nlu_interpreter` 
