...
* **Description**
  ...
  The matching is case sensitive by default and searches for matches of the keyword-string in the user
  message depending on `match_condition`. Supported match conditions are "contain" (default), "exact",
  "start" and "end". Match condition can be set in the `config.yml` file, intent metadata or example
  metadata.

  If the intent metadata `keyword_intent` is set to `true`, this classifier will reset any prior classification
  done by other classifiers. This is useful if you want to force the classification of the intent to be done
  by the keyword intent classifier.
  ...
  :::note
  This classifier is intended only for small projects or for certain channels which need to support
  slash commands (such as [Telegram](/connectors/telegram)). If you have few NLU training data, you
  can take a look at the recommended pipelines in [Tuning Your Model](./tuning-your-model.mdx).
...
* **Short**
  ...
  scores are ambiguous. The confidence is set to `1 - top intent confidence`.
