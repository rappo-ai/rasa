...
:::note
Any action that can be triggered with a forward slash uses [NLU Bypass](/nlu-training-data/#nlu-bypass).
For the trigger to work, NLU bypass must be enabled in the corresponding channel connector.
...
This action automatically triggers `action_session_start` as a follow-up action, and sets the
`session_started_metadata` slot with the value of the metadata of the "/restart" user message.
