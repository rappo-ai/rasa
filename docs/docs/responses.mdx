...
### Conditional Response Variations (experimental)
...
us make this feature ready for production.
:::
...
:::note
Because the feature is currently experimental, it is recommended to set the `influence_conversation`
slot setting to `False` for the slot mappings which are part of conditions.
:::
...
See [NLU Bypass](nlu-training-data/#nlu-bypass) to know more.
